...
def main():
    ...
    if args.checkpoint and args.config:
        print(
            "Warning: --config is ignored when --checkpoint is provided. Configuration file embedded in the checkpoint is used."
        )
...
def run_sample(device, ckpt_path, n_samples, k_gen):
    ...
    for _ in range(1):
        ph = 0.70  # Bernoulli probability
        h0 = (
            torch.bernoulli(torch.ones(n_samples, n_hidden) * ph)
            .float()
            .to(device)
        )  # bernoulli 0,1 with probability ph
        X0 = model.h_to_v(h0)

        """
        X0 = torch.ones(n_samples, n_visible) * 0
        transform = multinomial.OneHotTransform(n_class)
        X0 = transform(X0).view(-1).to(device)
        """

        """
        X0 = torch.randint(0, n_class, (n_samples, n_visible))
        transform = multinomial.OneHotTransform(n_class)
        X0 = transform(X0).view(-1).to(device)
        """
        """
        X0 = torch.randint(0, n_class, (n_samples, 1)).expand(n_samples, n_visible)
        transform = multinomial.OneHotTransform(n_class)
        X0 = transform(X0).view(-1).to(device)
        """
        ...
        if mc == "gibbs":
            with torch.no_grad():  # no graph, avoid GPU memory growth
                X_gen = model.forward(
                    X0, mc="gibbs", k=k_gen
                ).detach()  # gibbs or manual diff langevin does not need graph
        else:
            X_gen = model.forward(
                X0, mc="langevin", k=k_gen, epsilon=epsilon
            ).detach()  # langevin autodiff must use graph, GPU memory cautious
        # X_gen = X_gen.clamp(0,1)

        # Save Reconstructed values as a file in samples directory
        np.save(
            path.join(samples_dir, f"gen_n{n_samples}_k{k_gen}.npy"),
            X_gen.cpu(),
        )
        print(
            f"Numpy file, 'gen_n{n_samples}_k{k_gen}.npy', saved to {samples_dir}"
        )
